...
const
  MyRequestCard = () => {
    ...
    var min_price = shipment?.bids[0]?.bid_amount;
    var is_recommended_id = 0;

    useEffect(() => {
      shipment?.bids.map((bid) => {
        console.log('bid', bid);
        if (parseFloat(bid?.bid_amount) < min_price) {
          min_price = parseFloat(bid?.bid_amount);
          is_recommended_id = bid?.id;
        }
      });
    }, [shipment])
                    ...
                    shipment?.bids.map((bid: any) => {
                      return (
                        <div key={bid.id}>
                          ...
                          <div className="lg:w-[300px] w-full bg-black bg-opacity-25 p-2 border-b">
                          {bid?.id==shipment?.recomm_id && (
                            <div className="bg-yellow-600 text-white rounded-sm px-3 w-fit my-3 py-1 text-sm">Recommended</div>
                          )}
                            <div className="flex items-center gap-2">
